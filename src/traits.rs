...
pub trait ArrayReduction<T: NumericElement>: Sized {
    ...
    fn mean(&self) -> T where T: std::ops::Div<Output = T> + From<usize>;
    
    /// Mean along specific axis
    fn mean_axis(&self, axis: Option<usize>) -> Result<Self>;
    
    /// Standard deviation
    fn std(&self) -> T where T: FloatingPoint;
    ...
    /// Argument of minimum
    fn argmin(&self) -> usize where T: PartialOrd;
    
    /// Argument of maximum
    fn argmax(&self) -> usize where T: PartialOrd;
}
...
pub trait ArrayMath<T: NumericElement>: ArrayOps<T> {
    ...
    fn abs(&self) -> Self::Output where T: num_traits::Signed;
    
    /// Element-wise square root
    fn sqrt(&self) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise exponential
    fn exp(&self) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise natural logarithm
    fn ln(&self) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise sine
    fn sin(&self) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise cosine
    fn cos(&self) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise tangent
    fn tan(&self) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise power
    fn pow(&self, exponent: T) -> Self::Output where T: FloatingPoint;
    
    /// Element-wise power with another array
    fn pow_array(&self, exponents: &Self) -> Result<Self::Output> where T: FloatingPoint;
}
...
pub trait MemoryAllocator: Send + Sync + std::fmt::Debug {
    ...
    unsafe fn deallocate(&self, ptr: std::ptr::NonNull<u8>, layout: std::alloc::Layout) -> Result<()>;
    ...
    fn statistics(&self) -> Option<AllocationStats> { None }
    ...
    fn preferred_alignment(&self) -> usize { std::mem::align_of::<usize>() }
}
...
pub trait ArrayAllocator {
    ...
    fn allocate_array<T>(&self, len: usize) -> std::result::Result<std::ptr::NonNull<T>, Self::Error>;
    
    /// Allocate SIMD-aligned memory for vectorized operations
    fn allocate_simd_aligned<T>(&self, len: usize, alignment: usize) -> std::result::Result<std::ptr::NonNull<T>, Self::Error>;
    
    /// Allocate memory with cache-line alignment
    fn allocate_cache_aligned<T>(&self, len: usize) -> std::result::Result<std::ptr::NonNull<T>, Self::Error> {
...
pub trait AllocationStrategy: Send + Sync + std::fmt::Debug {
    ...
    fn select_allocator(&self, requirements: &AllocationRequirements) -> Box<dyn SpecializedAllocator<Error = crate::error::NumRs2Error>>;
    
    /// Get strategy statistics
    fn strategy_stats(&self) -> StrategyStats { StrategyStats::default() }
}
...
pub trait MemoryAware {
    ...
    fn set_allocator(&mut self, allocator: Box<dyn SpecializedAllocator<Error = crate::error::NumRs2Error>>);
...
impl NumericElement for f32 {
    ...
    fn from_f64(val: f64) -> Option<Self> { Some(val as f32) }
}
...
impl NumericElement for i32 {
    ...
    fn to_f64(&self) -> Option<f64> { Some(*self as f64) }
    fn from_f64(val: f64) -> Option<Self> { Some(val as i32) }
}
...
impl NumericElement for i64 {
    ...
    fn is_zero(&self) -> bool { *self == 0 }
    fn to_f64(&self) -> Option<f64> { Some(*self as f64) }
    fn from_f64(val: f64) -> Option<Self> { Some(val as i64) }
}
...
impl IntegerElement for i32 {
    ...
    fn saturating_mul(&self, other: &Self) -> Self { (*self).saturating_mul(*other) }
}
...
impl<T: FloatingPoint> NumericElement for Complex<T> {
    fn zero() -> Self { Complex::new(<T as NumericElement>::zero(), <T as NumericElement>::zero()) }
    fn one() -> Self { Complex::new(<T as NumericElement>::one(), <T as NumericElement>::zero()) }
    fn is_zero(&self) -> bool { NumericElement::is_zero(&self.re) && NumericElement::is_zero(&self.im) }
    fn to_f64(&self) -> Option<f64> { NumericElement::to_f64(&self.re) }
    fn from_f64(val: f64) -> Option<Self> { 
