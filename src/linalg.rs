    ...
    T: Float + Clone + Debug + ndarray_linalg::Lapack,
{
    ...
    pub fn inv(&self) -> Result<Array<T>> {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            if max_error > acceptable_error {
                eprintln!("Warning: Matrix inversion may be numerically unstable. Max error: {}", max_error);
            }
        }
    ...
    pub fn solve(&self, b: &Array<T>) -> Result<Array<T>> {
        ...
        if n == 1 {
            ...
            let a11 = a_data[0]; let a12 = a_data[1]; let a13 = a_data[2];
            let a21 = a_data[3]; let a22 = a_data[4]; let a23 = a_data[5];
            let a31 = a_data[6]; let a32 = a_data[7]; let a33 = a_data[8];
...
pub fn norm<T: Float + Clone + std::fmt::Display + std::ops::AddAssign>(a: &Array<T>, ord: Option<T>) -> Result<T> {
    let shape = a.shape();
    ...
    if shape.len() == 1 {
        ...
        if ord == T::from(1.0).unwrap() {
            ...
            if m == 2 && n == 2 {
                ...
                if data[0] == T::zero() && data[3] == T::zero() &&
            ...
            for _ in 0..max_iter {
                ...
                let y_data = y.to_vec();
                let max_abs = y_data.iter().fold(T::zero(), |acc, &val| T::max(acc, val.abs()));
                ...
                let x_flat = if x.ndim() > 1 { x.flatten(None) } else { x.clone() };
                let y_flat = if y.ndim() > 1 { y.flatten(None) } else { y.clone() };
...
#[cfg(feature = "matrix_decomp")]
pub fn matrix_rank<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>, tol: Option<T>) -> Result<usize> {
    // Check that the matrix is 2D
    let shape = a.shape();
    if shape.len() != 2 {
        return Err(NumRs2Error::DimensionMismatch(
...
/// Compute the QR decomposition of a matrix
#[cfg(feature = "matrix_decomp")]
pub fn qr<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>) -> Result<(Array<T>, Array<T>)> {
    a.qr()
}
...
/// Compute the Cholesky decomposition of a matrix
#[cfg(feature = "matrix_decomp")]
pub fn cholesky<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>) -> Result<Array<T>> {
    a.cholesky()
}
...
#[cfg(feature = "matrix_decomp")]
pub fn eig<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>, sort: Option<&str>) -> Result<(Array<T>, Array<T>)> {
...
#[cfg(feature = "scirs")]
pub fn solve<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>, b: &Array<T>) -> Result<Array<T>> {
    a.solve(b)
}
...
/// Compute the singular value decomposition of a matrix
#[cfg(feature = "matrix_decomp")]
pub fn svd<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>) -> Result<(Array<T>, Array<T>, Array<T>)> {
    a.svd()
}
...
#[cfg(feature = "matrix_decomp")]
pub fn pinv<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>, rcond: Option<T>) -> Result<Array<T>> {
    // Check that the matrix is 2D
    let shape = a.shape();
    if shape.len() != 2 {
        return Err(NumRs2Error::DimensionMismatch(
...
pub fn matrix_power<T: Float + Clone + Debug + ndarray_linalg::Lapack>(a: &Array<T>, n: i32) -> Result<Array<T>> {
    // Check if the matrix is square
    let shape = a.shape();
    if shape.len() != 2 || shape[0] != shape[1] {
        return Err(NumRs2Error::DimensionMismatch(
