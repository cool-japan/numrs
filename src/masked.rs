...
impl<T: Clone> MaskedArray<T> {
    ...
    /// ```
    pub fn masked_where(data: Array<T>, condition: Array<bool>, fill_value: Option<T>) -> Result<Self>
    where T: Clone + Default
    {
    ...
    where T: Clone
    {
        ...
        let mask_value = mask_array.get(indices).ok_or_else(|| {
            NumRs2Error::IndexOutOfBounds(
                format!("Failed to get mask at indices {:?}", indices)
            )
        })?;
        ...
        if *mask_value {
            ...
            let data_value = data_array.get(indices).ok_or_else(|| {
                NumRs2Error::IndexOutOfBounds(
                    format!("Failed to get data at indices {:?}", indices)
                )
            })?;
