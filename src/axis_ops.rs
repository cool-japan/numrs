...
pub trait AxisOps<T> {
    ...
    fn prod_axis(&self, axis: Option<usize>) -> Result<Array<T>> where T: Mul<Output = T> + One;
    
    /// Cumulative sum along specified axis
    fn cumsum_axis(&self, axis: usize) -> Result<Array<T>>;
    
    /// Cumulative product along specified axis
    fn cumprod_axis(&self, axis: usize) -> Result<Array<T>> where T: Mul<Output = T> + One;
    ...
    fn var_axis(&self, axis: Option<usize>) -> Result<Array<T>> where T: Float;
    
    /// Standard deviation along specified axis
    fn std_axis(&self, axis: Option<usize>) -> Result<Array<T>> where T: Float;
}
            ...
            Some(ax) => {
                if ax >= self.ndim() {
                    return Err(NumRs2Error::DimensionMismatch(
                        format!("Axis {} out of bounds for array of dimension {}", ax, self.ndim())
                    ));
                }
        ...
        match axis {
            Some(ax) => {
                ...
                let divisor = T::from(axis_size).ok_or_else(|| {
                    NumRs2Error::ConversionError("Failed to convert axis size to array type".to_string())
                })?;
            ...
            None => {
                ...
                let divisor = T::from(total_size).ok_or_else(|| {
                    NumRs2Error::ConversionError("Failed to convert array size to array type".to_string())
                })?;
    ...
    fn min_axis(&self, axis: Option<usize>) -> Result<Array<T>> {
        ...
        match axis {
            ...
            None => {
                ...
                let first = self.array().first().cloned().unwrap_or_else(|| {
                    panic!("Empty array")
                });
    ...
    where T: Clone + Float
    {
        match axis {
            ...
            None => {
                ...
                let mean_val = mean.array().first().cloned().unwrap_or_else(|| {
    ...
    F: Fn(&Array<T>) -> U
{
    if axis >= array.ndim() {
        return Err(NumRs2Error::DimensionMismatch(
            format!("Axis {} out of bounds for array of dimension {}", axis, array.ndim())
        ));
    }
