...
pub fn optimize_scheduling(
    ...
    match strategy {
        ...
        SchedulingStrategy::WorkStealing => work_stealing_scheduler(array_size, element_cost, num_threads),
...
fn static_scheduling(array_size: usize, element_cost: f64, num_threads: usize) -> usize {
    ...
    let cost_factor = (element_cost / 2.0).min(2.0).max(0.5);
...
fn dynamic_scheduling(array_size: usize, element_cost: f64, num_threads: usize) -> usize {
    ...
    // Scale based on element cost
    let cost_factor = (element_cost / 1.5).min(3.0).max(0.75);
    let scaled_threads = (optimal_threads as f64 * cost_factor) as usize;
...
fn guided_scheduling(array_size: usize, element_cost: f64, num_threads: usize) -> usize {
    ...
    // Scale based on element cost
    let cost_factor = (element_cost / 1.75).min(2.5).max(0.6);
    let scaled_threads = (optimal_threads as f64 * cost_factor) as usize;
...
pub fn work_stealing_scheduler(array_size: usize, element_cost: f64, num_threads: usize) -> usize {
    ...
    // Scale based on element cost
    let cost_factor = element_cost.min(4.0).max(0.25);
    let scaled_threads = (optimal_threads as f64 * cost_factor) as usize;
...
mod tests {
    ...
    fn test_optimize_scheduling() {
        ...
        // Compare strategies
        let static_threads = optimize_scheduling(100_000, 1.0, SchedulingStrategy::Static, num_threads);
        let dynamic_threads = optimize_scheduling(100_000, 1.0, SchedulingStrategy::Dynamic, num_threads);
        let guided_threads = optimize_scheduling(100_000, 1.0, SchedulingStrategy::Guided, num_threads);
