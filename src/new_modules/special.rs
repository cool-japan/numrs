    ...
    T: Float + Debug
{
    ...
    if x < T::one() {
        ...
        if n == 0 {
            ...
            for k in 1..16 {  // Usually 15 terms is enough for good precision
            ...
            if n == 1 {
                ...
                for k in 1..16 {
                    ...
                    let term = T::from(0.5).unwrap() * x * x / T::from(4.0).unwrap() *
                               T::from(k).unwrap() / (k_t * k_t * (k_t + T::one()));
                    k1 = k1 + term;
    ...
    // Large argument case (x ≥ 8*n)
        ...
        for k in 1..5 {  // Usually 4 terms provide excellent accuracy for large x
            let k_t = T::from(k).unwrap();
