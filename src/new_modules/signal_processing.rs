...
impl SignalProcessor {
    ...
    pub fn filter<T>(signal: &Array<T>, filter_type: FilterType, params: FilterParams<T>) -> Result<Array<T>>
    where
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        match filter_type {
            FilterType::LowPass => Self::lowpass_filter(signal, params.cutoff, params.sample_rate),
            FilterType::HighPass => Self::highpass_filter(signal, params.cutoff, params.sample_rate),
            FilterType::BandPass => Self::bandpass_filter(signal, params.low_cutoff, params.high_cutoff, params.sample_rate),
            FilterType::BandStop => Self::bandstop_filter(signal, params.low_cutoff, params.high_cutoff, params.sample_rate),
    ...
    /// Band-pass filter
    fn bandpass_filter<T>(signal: &Array<T>, low_cutoff: T, high_cutoff: T, sample_rate: T) -> Result<Array<T>>
    where
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        let fft_result = FFT::fft(signal)?;
    ...
    fn bandstop_filter<T>(signal: &Array<T>, low_cutoff: T, high_cutoff: T, sample_rate: T) -> Result<Array<T>>
    where
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        let fft_result = FFT::fft(signal)?;
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        let order = params.order.unwrap_or(4);
        let ripple = params.ripple.unwrap_or(<T as NumCast>::from(0.5).unwrap_or(T::zero()));
        
        // Apply Chebyshev response (simplified)
        for (i, &freq) in freq_data.iter().enumerate() {
            let normalized_freq = freq.abs() / params.cutoff;
            let epsilon = (<T as NumCast>::from(10.0).unwrap().powf(ripple / <T as NumCast>::from(10.0).unwrap()) - T::one()).sqrt();
            
            let response = if normalized_freq <= T::one() {
                T::one() / (T::one() + epsilon * epsilon * Self::chebyshev_poly(order, normalized_freq)).sqrt()
            } else {
                T::one() / (T::one() + epsilon * epsilon * (normalized_freq.powi(2 * order) - T::one())).sqrt()
            };
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        let product: Vec<Complex<T>> = fft1_data.iter().zip(fft2_data.iter())
            .map(|(a, b)| a * b)
            .collect();
    ...
    pub fn spectrogram<T>(signal: &Array<T>, window_size: usize, overlap: usize, window_type: &str) -> Result<SpectrogramResult<T>>
    where
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        if step == 0 {
            return Err(NumRs2Error::InvalidOperation("Window overlap cannot equal window size".to_string()));
        }
    ...
    pub fn cwt<T>(signal: &Array<T>, scales: &[T], wavelet_type: WaveletType) -> Result<Array<Complex<T>>>
    where
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        let signal_data = signal.to_vec();
        ...
        for &scale in scales {
            ...
            let complex_row: Vec<Complex<T>> = conv_data.iter()
                .take(n)
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        for i in 1..n/2 {
            fft_data[i] = fft_data[i] * Complex::new(T::zero(), -T::one());
        }
        for i in (n/2 + 1)..n {
            fft_data[i] = fft_data[i] * Complex::new(T::zero(), T::one());
        }
        ...
        // fft_data[0] and fft_data[n/2] remain unchanged
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        let sum_xy: f64 = signal_data.iter().enumerate()
        ...
        // Remove trend
        let detrended: Vec<T> = signal_data.iter().enumerate()
            .map(|(i, &y)| {
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        for i in 0..new_length {
            ...
            if idx >= old_length - 1 {
                ...
                let interpolated = val1 + (val2 - val1) * <T as NumCast>::from(frac).unwrap_or(T::zero());
...
mod tests {
    ...
    fn test_spectrogram() {
        ...
        assert_eq!(spec_result.time_axis.shape()[0], spec_result.spectrogram.shape()[0]);
        assert_eq!(spec_result.freq_axis.shape()[0], spec_result.spectrogram.shape()[1]);
    }

    #[test]
    #[ignore = "Hilbert transform implementation needs to be fixed - not producing correct imaginary components"]
    fn test_hilbert_transform() {
        ...
        for &mag in &magnitudes {
            assert!(mag >= 0.0 && mag <= 2.0, "Magnitude {} is out of reasonable range", mag);
        ...
        assert!(has_imag, "Analytic signal should have non-zero imaginary parts");
    }
}