...
impl FrequencyAnalyzer {
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        let window_power: f64 = window_values.iter()
            .map(|&w: &T| w.into())
            .map(|w: f64| w * w)
            .sum();
            ...
            *psd_val = *psd_val / segments_f;
            ...
            match scaling {
                PSDScaling::Density => {
                    ...
                    *psd_val = *psd_val / (sample_rate * <T as NumCast>::from(window_power).unwrap_or(T::one()));
                },
                PSDScaling::Spectrum => {
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        for i in 0..n_segments {
            ...
            for k in 0..n_freqs {
                ...
                psd1_accumulator[k] = psd1_accumulator[k] + Complex::<T>::new(f1.norm_sqr(), T::zero());
                psd2_accumulator[k] = psd2_accumulator[k] + Complex::<T>::new(f2.norm_sqr(), T::zero());
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        for i in 0..n_segments {
            ...
            let n_freqs = nfft / 2 + 1;
            for k in 0..n_freqs {
                cross_psd_accumulator[k] = cross_psd_accumulator[k] + fft1_data[k] * fft2_data[k].conj();
            }
    ...
    pub fn periodogram<T>(signal: &Array<T>, window: Option<&str>, scaling: PSDScaling) -> Result<PeriodogramResult<T>>
    where
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        for k in 0..n_tapers {
            ...
            // Normalize
            let norm: f64 = taper.iter().map(|&x| x.into()).map(|x: f64| x * x).sum::<f64>().sqrt();
            if norm > 0.0 {
        ...
        T: Float + Clone + Debug + From<f64>,
    {
        match window_type.to_lowercase().as_str() {
            ...
            "rectangular" | "boxcar" => {
                Ok(vec![<T as NumCast>::from(1.0).unwrap_or(T::zero()); n])
            },
            "kaiser" => {
    ...
    fn modified_bessel_i0(x: f64) -> f64 {
        ...
        if x.abs() < 3.75 {
            ...
            let inv_t = 1.0 / t;
            (x.abs().exp() / x.abs().sqrt()) * (0.39894228 + 0.01328592 * inv_t
                + 0.00225319 * inv_t * inv_t - 0.00157565 * inv_t * inv_t * inv_t
...
mod tests {
    ...
    fn test_periodogram() {
        ...
        let input = Array::from_vec(signal);
        let result = FrequencyAnalyzer::periodogram(&input, Some("hann"), PSDScaling::Density).unwrap();
    ...
    fn test_coherence() {
        ...
        let result = FrequencyAnalyzer::coherence(
            &input1,
            &input2,
            Some(64),
            Some(32),
    ...
    fn test_window_functions() {
        ...
        let rectangular = FrequencyAnalyzer::generate_window_function::<f64>(n, "rectangular").unwrap();
