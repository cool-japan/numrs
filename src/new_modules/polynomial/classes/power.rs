...
//! existing [`super::super::utils::polycompanion`] after reversing to that
//! sibling's descending-order convention. The sibling type's `derivative`/
...
/// Reverses `c` to the sibling [`super::super::core::Polynomial`]'s
/// descending-order convention and reuses its existing, separately-tested
/// [`super::super::utils::polycompanion`] rather than re-deriving the
/// standard (unscaled) companion matrix here.
pub(crate) fn companion<T: Float + Debug>(c: &[T]) -> Result<Array<T>> {
    let descending: Vec<T> = c.iter().rev().cloned().collect();
