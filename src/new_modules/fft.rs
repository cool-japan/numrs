...
impl FFT {
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        if !is_power_of_two(n) {
            return Err(NumRs2Error::InvalidOperation(
                format!("FFT requires input length to be a power of 2, got {}", n),
            ));
        }
        ...
        let mut complex_data: Vec<Complex<T>> = data.iter()
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        if !is_power_of_two(n) {
            return Err(NumRs2Error::InvalidOperation(
                format!("IFFT requires input length to be a power of 2, got {}", n),
            ));
        }
        ...
        let complex_data = complex_data_mut.iter()
        ...
        T: Float + Clone + Debug + From<f64>,
    {
        ...
        let half_n = n / 2;
        for i in 0..half_n {
            freqs.push(<T as NumCast>::from(i as f64).unwrap_or(T::zero()) * sample_rate / <T as NumCast>::from(n as f64).unwrap_or(T::zero()));
        }
        
        // Negative frequencies
        for i in half_n..n {
            freqs.push((<T as NumCast>::from(i as f64).unwrap_or(T::zero()) - <T as NumCast>::from(n as f64).unwrap_or(T::zero())) * sample_rate / <T as NumCast>::from(n as f64).unwrap_or(T::zero()));
        }
    ...
    pub fn fftshift<T: Clone>(x: &Array<T>) -> Result<Array<T>> {
        ...
        match ndim {
            ...
            _ => Err(NumRs2Error::InvalidOperation(
                format!("fftshift only supports 1D and 2D arrays, got {}D", ndim)
            )),
        }
    }
    ...
    pub fn ifftshift<T: Clone>(x: &Array<T>) -> Result<Array<T>> {
        ...
        match ndim {
            ...
            _ => Err(NumRs2Error::InvalidOperation(
                format!("ifftshift only supports 1D and 2D arrays, got {}D", ndim)
            )),
        }
    }
        ...
        T: Float + Clone + Debug + Into<f64> + From<f64>,
    {
        ...
        if n < 2 * (rfft_size - 1) {
            return Err(NumRs2Error::InvalidOperation(
                format!("IRFFT: n must be at least 2*(rfft_size-1), got n={} and rfft_size={}", n, rfft_size)
            ));
        }
        ...
        let real_data: Vec<T> = ifft_data.iter()
            .map(|c| c.re)
            .collect();
...
mod tests {
    ...
    fn complex_array(real: Vec<f64>, imag: Vec<f64>) -> Array<Complex64> {
        let complexes: Vec<Complex64> = real.iter()
            .zip(imag.iter())
