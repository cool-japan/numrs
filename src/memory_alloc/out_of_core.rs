...
impl<T: Copy> DataChunk<T> {
    ...
    fn load_from_disk(&mut self) -> Result<()> {
        ...
        let disk_path = self.metadata.disk_path.as_ref()
            .ok_or_else(|| NumRs2Error::InvalidOperation("No disk path for chunk".to_string()))?;
...
impl CacheManager {
    ...
    fn get_eviction_candidate(&mut self, strategy: CacheStrategy) -> Option<usize> {
        match strategy {
            ...
            CacheStrategy::LFU => {
                let min_chunk = self.access_frequency
                    .iter()
...
impl<T: Copy + Send + Sync + Default + 'static> OutOfCoreArray<T> {
    ...
    pub fn num_chunks(&self) -> usize {
        (self.total_elements + self.config.chunk_size - 1) / self.config.chunk_size
    }
    ...
    pub fn get_linear(&self, linear_index: usize) -> Result<T> {
        if linear_index >= self.total_elements {
            return Err(NumRs2Error::IndexOutOfBounds(
                format!("Linear index {} out of bounds (size {})", linear_index, self.total_elements)
            ));
        }
        ...
        if chunk_offset >= data.len() {
            return Err(NumRs2Error::IndexOutOfBounds(
                format!("Chunk offset {} out of bounds (chunk size {})", chunk_offset, data.len())
            ));
        }
    ...
    pub fn set_linear(&mut self, linear_index: usize, value: T) -> Result<()> {
        ...
        // Modify the data
        {
            ...
            let chunk = chunks.get_mut(&chunk_index)
                .ok_or_else(|| NumRs2Error::InvalidOperation("Chunk not found".to_string()))?;
    ...
    fn ensure_chunk_loaded(&self, chunk_index: usize) -> Result<()> {
        ...
        // Load the chunk
        {
            ...
            if let Some(chunk) = chunks.get_mut(&chunk_index) {
                ...
                let chunk_data = vec![T::default(); 
                    std::cmp::min(self.config.chunk_size, 
    ...
    pub fn get_cache_stats(&self) -> CacheStats {
        ...
        let chunks_on_disk = chunks.values().filter(|c| c.metadata.disk_path.is_some()).count();
