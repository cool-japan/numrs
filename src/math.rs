...
/// Calculate the discrete difference along the given axis with optional prepend/append
///
/// # Parameters
///
/// * `array` - Input array
...
/// * `prepend` - Optional values to prepend before computing differences
/// * `append` - Optional values to append before computing differences
///
/// # Returns
///
/// The n-th differences with optional prepend/append values.
///
/// # Examples
///
/// ```
...
/// // Without prepend/append - output has length - 1
/// let d = diff_extended(&a, 1, None, None, None).unwrap();
/// assert_eq!(d.to_vec(), vec![1.0, 2.0, 3.0, -7.0]);
///
/// // With prepend to match original length
/// let d_prepend = diff_extended(&a, 1, None, Some(&Array::from_vec(vec![0.0])), None).unwrap();
/// assert_eq!(d_prepend.size(), 5);
/// ```
    ...
    match axis {
        None => {
            ...
            for (i, val) in data.iter().enumerate() {
                if !val.is_nan() && (max_val.is_none() || *val > max_val.unwrap()) {
                    max_val = Some(*val);
                    max_idx = i;
                }
            }
    ...
    T: Float + Clone + Zero,
{
    ...
    match axis {
        None => {
            ...
            for (i, val) in data.iter().enumerate() {
                if !val.is_nan() && (min_val.is_none() || *val < min_val.unwrap()) {
                    min_val = Some(*val);
                    min_idx = i;
                }
            }
...
where
    T: Float + Clone + std::ops::Mul<Output = T> + Zero + One,
{
    if let Some(ax) = axis {
        let ax = if ax < 0 {
            (array.ndim() as isize + ax) as usize
