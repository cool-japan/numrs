...
impl Generator {
    ...
    pub fn normal<T: Float + NumCast + Clone + std::fmt::Debug + std::fmt::Display>(&self, mean: T, std: T, shape: &[usize]) -> Result<Array<T>> {
        if std <= T::zero() {
    ...
    pub fn lognormal<T: Float + NumCast + Clone + std::fmt::Debug + std::fmt::Display>(&self, mean: T, sigma: T, shape: &[usize]) -> Result<Array<T>> {
        if sigma <= T::zero() {
            return Err(NumRs2Error::InvalidOperation(
    ...
    pub fn bernoulli<T: Float + NumCast + Clone + std::fmt::Debug + std::fmt::Display>(&self, p: T, shape: &[usize]) -> Result<Array<T>> {
        if p < T::zero() || p > T::one() {
            return Err(NumRs2Error::InvalidOperation(
    ...
    pub fn gamma<T: Float + NumCast + Clone + std::fmt::Debug + std::fmt::Display>(&self, shape_param: T, scale: T, size_shape: &[usize]) -> Result<Array<T>> {
    ...
    pub fn exponential<T: Float + NumCast + Clone + std::fmt::Debug + std::fmt::Display>(&self, scale: T, shape: &[usize]) -> Result<Array<T>> {
        if scale <= T::zero() {
            return Err(NumRs2Error::InvalidOperation(
...
pub fn randn<T: Float + NumCast + Clone + std::fmt::Debug + std::fmt::Display>(shape: &[usize]) -> Result<Array<T>> {
    let generator = GLOBAL_GENERATOR.lock().map_err(|_| {
        NumRs2Error::InvalidOperation("Failed to acquire global generator lock".to_string())
    })?;
...
pub fn uniform<T: Clone + PartialOrd + SampleUniform>(low: T, high: T, shape: &[usize]) -> Result<Array<T>> {
    let generator = GLOBAL_GENERATOR.lock().map_err(|_| {
        NumRs2Error::InvalidOperation("Failed to acquire global generator lock".to_string())
    })?;
...
pub fn choice<T: Clone>(array: &Array<T>, size: Option<usize>, replace: Option<bool>) -> Result<Array<T>> {
    let generator = GLOBAL_GENERATOR.lock().map_err(|_| {
        NumRs2Error::InvalidOperation("Failed to acquire global generator lock".to_string())
    })?;
