...
use numrs2::new_modules::matrix_decomp::{svd, lu, qr, cholesky, pivoted_cholesky, condition_number};
...
fn test_condition_number_accuracy() {
    ...
    assert!(cond_near_singular > 1e2 && cond_near_singular < 1e5 || cond_near_singular.is_infinite(),
           "Expected condition number to be high, got {}", cond_near_singular);
    ...
    assert!(cond_hilbert > 1e4, 
           "Hilbert matrix should have condition number > 1e4, got {}", cond_hilbert);
}
...
fn test_decomposition_stability_well_conditioned() {
    ...
    assert!(max_lu_error < 1e-10, "LU decomposition error: {}", max_lu_error);
    ...
    assert!(max_qr_error < 1e-10, "QR decomposition error: {}", max_qr_error);
    
    // Test Cholesky decomposition
    let l_chol = cholesky(&a).unwrap();
    ...
    assert!(max_chol_error < 1e-10, "Cholesky decomposition error: {}", max_chol_error);
    ...
    for i in 0..n {
        for j in (i+1)..n {
            ...
            assert_eq!(val, 0.0, "Pivoted Cholesky should produce a lower triangular matrix");
        }
    }
}
...
fn test_decomposition_stability_ill_conditioned() {
    ...
    assert!(max_svd_error < expected_max_error, 
            "SVD decomposition error: {}, expected < {}", max_svd_error, expected_max_error);
    ...
    assert!(max_lu_error < expected_max_error * 10.0, 
            "LU decomposition error: {}, expected < {}", max_lu_error, expected_max_error * 10.0);
    ...
    assert!(max_qr_error < expected_max_error * 10.0, 
            "QR decomposition error: {}, expected < {}", max_qr_error, expected_max_error * 10.0);
    ...
    println!("QR orthogonality error: {}, using tolerance: {}", max_ortho_error, ortho_tol);
    assert!(max_ortho_error < ortho_tol, 
            "QR: Q is not sufficiently orthogonal. Max error: {}", max_ortho_error);
    ...
    println!("Cholesky error: {}, expected < {}", max_chol_error, expected_max_error * 100.0);
    // For extremely ill-conditioned matrices like Hilbert, relaxed tolerance
    assert!(max_chol_error < 1.0, 
            "Cholesky decomposition error: {}", max_chol_error);
}
...
fn test_pivoted_cholesky_vs_standard() {
    ...
    let l_std = cholesky(&a_symm).unwrap();
    
    // Check it's lower triangular
    for i in 0..n {
        for j in (i+1)..n {
            assert_eq!(l_std.get(&[i, j]).unwrap(), 0.0, 
    ...
    for i in 0..n {
        for j in (i+1)..n {
            assert_eq!(l_piv.get(&[i, j]).unwrap(), 0.0, 
...
fn test_decompositions_with_scaling() {
    ...
    let (u, s, vt) = svd(&large_matrix).unwrap();
    let mut s_diag = Array::zeros(&[n, n]);
    for i in 0..s.size() {
        s_diag.set(&[i, i], s.get(&[i]).unwrap()).unwrap();
    }
    ...
    assert!(max_svd_error < 1e-10, "SVD relative error with scaling: {}", max_svd_error);
    
    // QR should handle scaling
    let (q, r) = qr(&large_matrix).unwrap();
    let qr_product = q.matmul(&r).unwrap();
    
    let mut max_qr_error = 0.0;
    for i in 0..n {
        for j in 0..n {
            let diff = (large_matrix.get(&[i, j]).unwrap() - qr_product.get(&[i, j]).unwrap()).abs();
            let rel_error = diff / large_matrix.get(&[i, j]).unwrap().abs();
            max_qr_error = max_qr_error.max(rel_error);
        }
    }
    assert!(max_qr_error < 1e-10, "QR relative error with scaling: {}", max_qr_error);
}
