...
fn load_reference_data() -> HashMap<String, HashMap<String, serde_json::Value>> {
    let data_path = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/py/array_operations_reference_data.json");
    let data_str = std::fs::read_to_string(data_path)
        .expect("Failed to read reference data file. Run the Python script first.");
    
    serde_json::from_str(&data_str)
        .expect("Failed to parse reference data JSON")
}
...
fn assert_arrays_close(actual: &Array<f64>, expected: &Array<f64>, tolerance: f64) {
    ...
    assert_eq!(actual_vec.len(), expected_vec.len(), "Array sizes don't match");
    ...
    fn test_zeros_creation() {
        ...
        for (test_name, test_data) in creation_tests {
            if test_name.starts_with("zeros_shape_") {
                ...
                let shape: Vec<usize> = test_obj["shape"].as_array().unwrap()
                    .iter().map(|v| v.as_u64().unwrap() as usize).collect();
                let expected: SerializedArray = serde_json::from_value(test_obj["result"].clone()).unwrap();
                
                if shape.iter().product::<usize>() > 0 {  // Skip empty arrays
    ...
    fn test_reshape_operations() {
        ...
        for (test_name, test_data) in manipulation_tests {
            if test_name.starts_with("reshape_") {
                let test_obj = test_data.as_object().unwrap();
                let input: SerializedArray = serde_json::from_value(test_obj["input"].clone()).unwrap();
                let new_shape: Vec<usize> = test_obj["new_shape"].as_array().unwrap()
                    .iter().map(|v| v.as_u64().unwrap() as usize).collect();
                let expected: SerializedArray = serde_json::from_value(test_obj["result"].clone()).unwrap();
    ...
    fn test_element_wise_arithmetic() {
        ...
        for (_test_name, test_data) in arithmetic_tests {
            ...
            if ["add", "subtract", "multiply", "divide"].contains(&operation) {
                let input1: SerializedArray = serde_json::from_value(test_obj["input1"].clone()).unwrap();
                let input2: SerializedArray = serde_json::from_value(test_obj["input2"].clone()).unwrap();
                let expected: SerializedArray = serde_json::from_value(test_obj["result"].clone()).unwrap();
    ...
    fn test_power_operations() {
        ...
        for (test_name, test_data) in math_tests {
            if test_name.starts_with("power_") {
                let test_obj = test_data.as_object().unwrap();
                let base: SerializedArray = serde_json::from_value(test_obj["base"].clone()).unwrap();
    ...
    fn test_element_wise_comparisons() {
        ...
        for (test_name, test_data) in comparison_tests {
            if test_name.ends_with("_arrays") {
                ...
                let input1: SerializedArray = serde_json::from_value(test_obj["input1"].clone()).unwrap();
                let input2: SerializedArray = serde_json::from_value(test_obj["input2"].clone()).unwrap();
                let expected: Vec<bool> = test_obj["result"].as_array().unwrap()
                ...
                assert_eq!(actual, expected, "Comparison operation {} failed", operation);
            }
        }
    }
    ...
    fn test_basic_indexing() {
        ...
        for (test_name, test_data) in indexing_tests {
            if test_name.starts_with("get_") {
                let test_obj = test_data.as_object().unwrap();
                let input: SerializedArray = serde_json::from_value(test_obj["input"].clone()).unwrap();
                let indices: Vec<usize> = test_obj["indices"].as_array().unwrap()
                    .iter().map(|v| v.as_u64().unwrap() as usize).collect();
                let expected = test_obj["result"].as_f64().unwrap();
