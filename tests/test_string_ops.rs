...
fn test_array_creation_functions() {
    ...
    assert_eq!(fixed_arr.get(&[0, 0]).unwrap().to_string().unwrap(), "hello");
    assert_eq!(fixed_arr.get(&[1, 1]).unwrap().to_string().unwrap(), "array");
...
fn test_string_arithmetic_operations() {
    ...
    assert_eq!(result.get(&[0]).unwrap().to_string().unwrap(), "hello world");
    assert_eq!(result.get(&[1]).unwrap().to_string().unwrap(), "good morning");
    ...
    assert_eq!(result.get(&[1]).unwrap().to_string().unwrap(), "goodgoodgood");
...
fn test_string_formatting_operations() {
    ...
    assert_eq!(result.get(&[0]).unwrap().to_string().unwrap(), "Value: 3.14159");
    assert_eq!(result.get(&[1]).unwrap().to_string().unwrap(), "Number: 42");
    assert_eq!(result.get(&[2]).unwrap().to_string().unwrap(), "Float: 2.71828");
}
...
fn test_string_justification_operations() {
    ...
    assert_eq!(centered.get(&[0]).unwrap().to_string().unwrap(), "**hello***");
    assert_eq!(centered.get(&[1]).unwrap().to_string().unwrap(), "***test***");
    assert_eq!(centered.get(&[2]).unwrap().to_string().unwrap(), "****a*****");
    
    // Test left justification
    let left_just = ljust(&arr, 8, Some('-')).unwrap();
    assert_eq!(left_just.get(&[0]).unwrap().to_string().unwrap(), "hello---");
    assert_eq!(left_just.get(&[1]).unwrap().to_string().unwrap(), "test----");
    assert_eq!(left_just.get(&[2]).unwrap().to_string().unwrap(), "a-------");
    
    // Test right justification
    let right_just = rjust(&arr, 8, Some('=')).unwrap();
    assert_eq!(right_just.get(&[0]).unwrap().to_string().unwrap(), "===hello");
    assert_eq!(right_just.get(&[1]).unwrap().to_string().unwrap(), "====test");
    assert_eq!(right_just.get(&[2]).unwrap().to_string().unwrap(), "=======a");
}
...
fn test_string_stripping_operations() {
    ...
    assert_eq!(char_stripped.get(&[0]).unwrap().to_string().unwrap(), "hello");
    assert_eq!(char_stripped.get(&[1]).unwrap().to_string().unwrap(), "world");
    assert_eq!(char_stripped.get(&[2]).unwrap().to_string().unwrap(), "test");
    
    // Test left strip
    let left_stripped = lstrip(&arr, None).unwrap();
    assert_eq!(left_stripped.get(&[0]).unwrap().to_string().unwrap(), "hello  ");
    
    // Test right strip  
    let right_stripped = rstrip(&arr, None).unwrap();
    assert_eq!(right_stripped.get(&[0]).unwrap().to_string().unwrap(), "  hello");
}
...
fn test_string_case_operations() {
    ...
    assert_eq!(upper_result.get(&[0]).unwrap().to_string().unwrap(), "HELLO WORLD");
    assert_eq!(upper_result.get(&[3]).unwrap().to_string().unwrap(), "ALREADY_LOWER");
    
    // Test lowercase
    let lower_result = lower(&arr).unwrap();
    assert_eq!(lower_result.get(&[1]).unwrap().to_string().unwrap(), "test case");
    assert_eq!(lower_result.get(&[2]).unwrap().to_string().unwrap(), "mixed case");
    
    // Test title case
    let title_result = title(&arr).unwrap();
    assert_eq!(title_result.get(&[0]).unwrap().to_string().unwrap(), "Hello World");
    assert_eq!(title_result.get(&[1]).unwrap().to_string().unwrap(), "Test Case");
    
    // Test capitalize
    let cap_result = capitalize(&arr).unwrap();
    assert_eq!(cap_result.get(&[0]).unwrap().to_string().unwrap(), "Hello world");
    assert_eq!(cap_result.get(&[2]).unwrap().to_string().unwrap(), "Mixed case");
    
    // Test swapcase
    let swap_result = char::swapcase(&arr).unwrap();
    assert_eq!(swap_result.get(&[0]).unwrap().to_string().unwrap(), "HELLO WORLD");
    assert_eq!(swap_result.get(&[1]).unwrap().to_string().unwrap(), "test case");
}
...
fn test_string_replacement_operations() {
    ...
    assert_eq!(replaced.get(&[1]).unwrap().to_string().unwrap(), "example case example");
    assert_eq!(replaced.get(&[2]).unwrap().to_string().unwrap(), "no match here");
    
    // Test replace with count limit
    let limited_replace = replace(&arr, "hello", "hi", Some(1)).unwrap();
    assert_eq!(limited_replace.get(&[0]).unwrap().to_string().unwrap(), "hi world hello");
}
...
fn test_string_split_join_operations() {
    ...
    assert_eq!(joined.get(&[0]).unwrap().to_string().unwrap(), "hello-world");
...
fn test_character_type_checks() {
    ...
    assert_eq!(alpha_result.to_vec(), vec![true, false, true, true, false, false, false]);
    
    // Test isdigit
    let digit_result = chartype::isdigit(&arr).unwrap();
    assert_eq!(digit_result.to_vec(), vec![false, true, false, false, false, false, false]);
    
    // Test isalnum
    let alnum_result = chartype::isalnum(&arr).unwrap();
    assert_eq!(alnum_result.to_vec(), vec![true, true, true, true, false, false, true]);
    
    // Test islower
    let lower_result = chartype::islower(&arr).unwrap();
    assert_eq!(lower_result.to_vec(), vec![true, false, false, false, false, false, true]);
    
    // Test isupper
    let upper_result = chartype::isupper(&arr).unwrap();
    assert_eq!(upper_result.to_vec(), vec![false, false, true, false, false, false, false]);
    
    // Test isspace
    let space_result = chartype::isspace(&arr).unwrap();
    assert_eq!(space_result.to_vec(), vec![false, false, false, false, false, true, false]);
}
...
fn test_title_case_checks() {
    ...
    assert_eq!(title_result.to_vec(), vec![true, false, false, false, false]);
}
...
fn test_partition_operations() {
    ...
    assert_eq!(partitions[1], ("one".to_string(), "-".to_string(), "two-three".to_string()));
    
    // Test rpartition
    let rpartitions = char::rpartition(&arr, "-").unwrap();
    assert_eq!(rpartitions[0], ("hello-world".to_string(), "-".to_string(), "test".to_string()));
    assert_eq!(rpartitions[1], ("one-two".to_string(), "-".to_string(), "three".to_string()));
    
    // Test with non-existent separator
    let no_sep_partition = char::partition(&arr, "@").unwrap();
    assert_eq!(no_sep_partition[0], ("hello-world-test".to_string(), "".to_string(), "".to_string()));
}
...
fn test_regex_operations() {
    ...
    assert_eq!(substituted.get(&[1]).unwrap().to_string().unwrap(), "testXXXexample");
    assert_eq!(substituted.get(&[2]).unwrap().to_string().unwrap(), "no_digits_here");
...
fn test_utility_functions() {
    ...
    assert_eq!(prefixed.get(&[0]).unwrap().to_string().unwrap(), "data_file1");
    assert_eq!(prefixed.get(&[2]).unwrap().to_string().unwrap(), "data_file3");
    
    // Test array_with_suffix
    let prefixes = vec!["image", "document", "archive"];
    let suffixed = array_with_suffix(&prefixes, ".txt", "U").unwrap();
    assert_eq!(suffixed.get(&[0]).unwrap().to_string().unwrap(), "image.txt");
    assert_eq!(suffixed.get(&[2]).unwrap().to_string().unwrap(), "archive.txt");
}
...
fn test_edge_cases() {
    ...
    assert_eq!(upper_single.to_vec().iter().map(|s| s.to_string().unwrap()).collect::<Vec<_>>(), 
               vec!["A", "B", "1"]);
...
fn test_unicode_handling() {
    ...
    assert_eq!(upper_unicode.get(&[0]).unwrap().to_string().unwrap(), "CAFÉ");
