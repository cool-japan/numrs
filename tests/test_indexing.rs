...
fn test_ellipsis_indexing() {
    ...
    let result1 = array.index(&[IndexSpec::Ellipsis, IndexSpec::Index(0)]).unwrap();
    ...
    let result2 = array.index(&[IndexSpec::Index(0), IndexSpec::Ellipsis]).unwrap();
    ...
    let result3 = array.index(&[IndexSpec::Ellipsis, IndexSpec::Index(0), IndexSpec::Index(0)]).unwrap();
...
fn test_fancy_indexing() {
    ...
    // Select rows 0 and 2
    let result1 = array.index(&[IndexSpec::Indices(vec![0, 2]), IndexSpec::All]).unwrap();
    ...
    let result2 = array.index(&[IndexSpec::All, IndexSpec::Indices(vec![1, 3])]).unwrap();
