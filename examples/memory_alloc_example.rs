use numrs2::memory_alloc::{
    ArenaAllocator, ArenaConfig,
    PoolAllocator, PoolConfig, 
    AlignedAllocator, AlignmentConfig,
    AllocStrategy, get_global_allocator_strategy, init_global_allocator, reset_global_allocator,
    get_default_allocator
};
use numrs2::memory_alloc::arena::ArenaVec;
use numrs2::memory_alloc::aligned::{AlignedBox, AlignedVec};
...
fn main() {
    ...
    println!("  Pool allocator: {} allocations in {:?} ({:?} per allocation)",
             1000, pool_alloc_time, pool_alloc_time / 1000);
    println!("  Standard allocator: {} allocations in {:?} ({:?} per allocation)",
             1000, std_alloc_time, std_alloc_time / 1000);
    println!("  Speed improvement: {:.2}x for allocation",
             std_alloc_time.as_nanos() as f64 / pool_alloc_time.as_nanos() as f64);
    
    println!("\n  Pool deallocator: {} deallocations in {:?} ({:?} per deallocation)",
             1000, pool_dealloc_time, pool_dealloc_time / 1000);
    println!("  Standard deallocator: {} deallocations in {:?} ({:?} per deallocation)",
             1000, std_dealloc_time, std_dealloc_time / 1000);
    println!("  Speed improvement: {:.2}x for deallocation",
    ...
    println!("  Arena: 10,000 small allocations (32 bytes) in {:?} ({:?} per allocation)",
             arena_alloc_time, arena_alloc_time / 10000);
    ...
    println!("  Standard: 10,000 small allocations (32 bytes) in {:?} ({:?} per allocation)",
             std_alloc_time, std_alloc_time / 10000);
    
    println!("  Speed improvement: {:.2}x for many small allocations",
    ...
    println!("\n  Arena reset (freeing all 10,000 allocations): {:?}", arena_reset_time);
    ...
    println!("  Standard deallocation (freeing 10,000 allocations): {:?}", std_dealloc_time);
    
    println!("  Speed improvement: {:.2}x for bulk deallocation",
    ...
    let aligned_ptr = aligned.allocate_array::<f32>(size).expect("Aligned allocation failed");
    ...
    // Demo aligned box
    println!("\nAligned Box demo:");
    let aligned_box = AlignedBox::new(Matrix4x4::identity(), 32)
        .expect("Aligned box creation failed");
    ...
    match strategy {
        ...
        AllocStrategy::Aligned => {
            ...
            for (ptr, size) in ptrs {
                unsafe { aligned.deallocate(ptr, size); }
            }
