...
fn main() -> Result<()> {
    ...
    let moderately_ill_conditioned = Array::from_vec(vec![1.0, 0.9999, 0.9999, 1.0]).reshape(&[2, 2]);
    ...
    println!("Is well-conditioned: {}", well_conditioned.is_well_conditioned().unwrap());
    
    println!("\nModerately ill-conditioned matrix:");
    println!("{}", moderately_ill_conditioned);
    println!("Condition number: {}", moderately_ill_conditioned.cond().unwrap());
    println!("Is well-conditioned: {}", moderately_ill_conditioned.is_well_conditioned().unwrap());
    
    println!("\nVery ill-conditioned matrix:");
    println!("{}", very_ill_conditioned);
    println!("Condition number: {}", very_ill_conditioned.cond().unwrap());
    println!("Is well-conditioned: {}", very_ill_conditioned.is_well_conditioned().unwrap());
