...
use numrs2::optimized_ops::{
    enhanced_math, enhanced_exp, simd_matmul, simd_vector_ops,
    get_optimization_info, SimdMathOps, process_large_array
};
...
fn benchmark_trigonometric_functions() {
    ...
    for &size in &sizes {
        ...
        let data = Array1::from_vec(
            (0..size).map(|x| x as f64 * 0.01).collect::<Vec<_>>()
        );
        let data_view = data.view();
        
        // Standard implementation
        let std_ops = benchmark_operation(
            "Standard sin (scalar)",
            size,
            100,
            || {
        ...
        #[cfg(feature = "scirs")]
        let par_ops = benchmark_operation(
            "Parallel sin",
            size,
            100,
            || {
...
fn benchmark_exponential_functions() {
    ...
    for &size in &sizes {
        ...
        let data = Array1::from_vec(
            (0..size).map(|x| (x as f64 * 0.01).min(10.0)).collect::<Vec<_>>()
        );
        let data_view = data.view();
        
        // Standard sqrt
        let std_ops = benchmark_operation(
            "Standard sqrt (scalar)",
            size,
            100,
            || {
        ...
        // SIMD sqrt
        #[cfg(feature = "scirs")]
        let simd_ops = benchmark_operation(
            "SIMD sqrt",
            size,
            100,
            || {
...
fn benchmark_matrix_operations() {
    ...
    for &size in &sizes {
        ...
        let a = Array2::from_shape_vec(
            (size, size),
            (0..size*size).map(|x| x as f32 * 0.01).collect::<Vec<_>>()
        ).unwrap();
        let b = Array2::from_shape_vec(
            (size, size),
            (0..size*size).map(|x| x as f32 * 0.02).collect::<Vec<_>>()
        ).unwrap();
...
fn benchmark_large_array_processing() {
    ...
    let data = Array1::from_vec(
        (0..size).map(|x| x as f64 * 0.001).collect::<Vec<_>>()
    );
    let data_view = data.view();
...
fn benchmark_adaptive_algorithms() {
    ...
    for &size in &sizes {
        let data = Array1::from_vec(
            (0..size).map(|x| (x + 1) as f64).collect::<Vec<_>>()
        );
        let data_view = data.view();
