...
fn main() {
    ...
    let time_fixed_high = benchmark_parallel_sum(&array_data, ParallelizationThreshold::Fixed(fixed_high));
    println!("  Fixed threshold ({}): {:?}", fixed_high, time_fixed_high);
    
    // Fixed threshold - too low
    let fixed_low = 1_000;
    let time_fixed_low = benchmark_parallel_sum(&array_data, ParallelizationThreshold::Fixed(fixed_low));
    ...
    println!("  Best threshold provides a {:.2}x speedup over worst threshold",
             time_fixed_high.max(time_fixed_low).max(time_adaptive).as_secs_f64() / best_time.as_secs_f64());
    ...
    // Static scheduling
    let static_threads = optimize_scheduling(size, element_cost, SchedulingStrategy::Static, num_threads);
    let time_static = benchmark_scheduling(&array_data, SchedulingStrategy::Static);
    println!("  Static scheduling (threads={}): {:?}", static_threads, time_static);
    
    // Dynamic scheduling
    let dynamic_threads = optimize_scheduling(size, element_cost, SchedulingStrategy::Dynamic, num_threads);
    let time_dynamic = benchmark_scheduling(&array_data, SchedulingStrategy::Dynamic);
    println!("  Dynamic scheduling (threads={}): {:?}", dynamic_threads, time_dynamic);
    
    // Guided scheduling
    let guided_threads = optimize_scheduling(size, element_cost, SchedulingStrategy::Guided, num_threads);
    let time_guided = benchmark_scheduling(&array_data, SchedulingStrategy::Guided);
    println!("  Guided scheduling (threads={}): {:?}", guided_threads, time_guided);
    
    // Work stealing scheduling
    let ws_threads = optimize_scheduling(size, element_cost, SchedulingStrategy::WorkStealing, num_threads);
    let time_ws = benchmark_scheduling(&array_data, SchedulingStrategy::WorkStealing);
    println!("  Work-stealing scheduling (threads={}): {:?}", ws_threads, time_ws);
    
    // Adaptive scheduling
    let adaptive_threads = optimize_scheduling(size, element_cost, SchedulingStrategy::Adaptive, num_threads);
    let time_adaptive = benchmark_scheduling(&array_data, SchedulingStrategy::Adaptive);
    println!("  Adaptive scheduling (threads={}): {:?}", adaptive_threads, time_adaptive);
    
    // Calculate speedup
    let best_sched_time = time_static.min(time_dynamic).min(time_guided).min(time_ws).min(time_adaptive);
    println!("  Best scheduling strategy provides a {:.2}x speedup over worst strategy",
             time_static.max(time_dynamic).max(time_guided).max(time_ws).max(time_adaptive).as_secs_f64() 
    ...
    let time_cache = benchmark_partitioning(&array_data, WorkloadPartitioning::CacheOptimizedChunks);
    println!("  Cache-optimized chunks: {:?}", time_cache);
    
    // Dynamic partitioning
    let time_dynamic = benchmark_partitioning(&array_data, WorkloadPartitioning::DynamicPartitioning);
    println!("  Dynamic partitioning: {:?}", time_dynamic);
    
    // Calculate speedup
    let best_part_time = time_equal.min(time_variable).min(time_pot).min(time_cache).min(time_dynamic);
    println!("  Best partitioning strategy provides a {:.2}x speedup over worst strategy",
             time_equal.max(time_variable).max(time_pot).max(time_cache).max(time_dynamic).as_secs_f64() 
    ...
    println!("    Minimum parallel size: {}", optimized_config.min_parallel_size);
    println!("    Chunk size: {}", optimized_config.chunk_size);
    println!("    Scheduling strategy: {:?}", optimized_config.scheduling_strategy);
    ...
    println!("  Best configuration provides a {:.2}x speedup over default",
    ...
    let optimal_threads_small = optimize_parallel_computation(small_size, 0.1, SchedulingStrategy::Adaptive);
    println!("  Small workload (size={}, cost=0.1):", small_size);
    println!("    Optimal threads: {}", optimal_threads_small);
    println!("    Parallel execution recommended: {}", optimal_threads_small > 1);
    ...
    let optimal_threads_medium = optimize_parallel_computation(medium_size, 1.0, SchedulingStrategy::Adaptive);
    println!("  Medium workload (size={}, cost=1.0):", medium_size);
    println!("    Optimal threads: {}", optimal_threads_medium);
    println!("    Parallel execution recommended: {}", optimal_threads_medium > 1);
    ...
    let optimal_threads_large = optimize_parallel_computation(large_size, 5.0, SchedulingStrategy::Adaptive);
    println!("  Large workload (size={}, cost=5.0):", large_size);
    println!("    Optimal threads: {}", optimal_threads_large);
    println!("    Parallel execution recommended: {}", optimal_threads_large > 1);
    
    // Very large workload with moderate operations
    let very_large_size = 10_000_000;
    let optimal_threads_very_large = optimize_parallel_computation(very_large_size, 0.5, SchedulingStrategy::Adaptive);
    println!("  Very large workload (size={}, cost=0.5):", very_large_size);
    println!("    Optimal threads: {}", optimal_threads_very_large);
    println!("    Parallel execution recommended: {}", optimal_threads_very_large > 1);
}

// Benchmark summation with different parallelization thresholds
fn benchmark_parallel_sum(data: &[f64], threshold_type: ParallelizationThreshold) -> std::time::Duration {
    ...
    let sum = if size <= threshold {
        ...
        data.iter().map(|&x| compute_with_cost(x, element_cost)).sum::<f64>()
    } else {
        // Parallel execution
        data.par_iter().map(|&x| compute_with_cost(x, element_cost)).sum::<f64>()
    };
    ...
    let sum = pool.install(|| {
        match strategy {
            SchedulingStrategy::Static => {
                ...
                chunks.par_iter()
                    .map(|chunk| chunk.iter().map(|&x| compute_with_cost(x, element_cost)).sum::<f64>())
                    .sum::<f64>()
            },
...
fn benchmark_partitioning(data: &[f64], partitioning: WorkloadPartitioning) -> std::time::Duration {
    ...
    let sum: f64 = partitions.par_iter().map(|range| {
        let mut sum = 0.0;
