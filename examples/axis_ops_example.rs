...
fn main() -> Result<()> {
    ...
    println!("Sum along axis 0 (columns) (shape {:?}):", sum_axis0.shape());
    println!("{:?}", sum_axis0);
    ...
    println!("Cumulative sum along axis 0 (shape {:?}):", cumsum_axis0.shape());
    println!("{:?}", cumsum_axis0);
    
    let cumsum_axis1 = arr.cumsum_axis(1)?;
    println!("Cumulative sum along axis 1 (shape {:?}):", cumsum_axis1.shape());
    println!("{:?}", cumsum_axis1);
    
    // Cumulative product along axes
    let cumprod_axis1 = arr.cumprod_axis(1)?;
    println!("Cumulative product along axis 1 (shape {:?}):", cumprod_axis1.shape());
    ...
    println!("Argmin along axis 1 (indices of minimum values): {:?}", argmin_axis1);
    
    let argmax_axis0 = arr.argmax_axis(0)?;
    println!("Argmax along axis 0 (indices of maximum values): {:?}", argmax_axis0);
