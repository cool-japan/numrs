...
use numrs2::optimized_ops::{
    enhanced_math, enhanced_exp, simd_matmul, simd_elementwise_ops,
...
impl BenchmarkResult {
    fn print_summary(&self, operation: &str, size: usize) {
        ...
        if let Some(simd_time) = self.simd_time {
            ...
            println!("SIMD time: {:>10.3} ms (speedup: {:.2}x)", simd_time * 1000.0, speedup);
        }
        
        #[cfg(feature = "gpu")]
        if let Some(gpu_time) = self.gpu_time {
            let speedup = self.cpu_time / gpu_time;
            println!("GPU time:  {:>10.3} ms (speedup: {:.2}x)", gpu_time * 1000.0, speedup);
        }
    }
}
...
fn benchmark_transcendental_functions(size: usize) -> Result<()> {
    ...
    let data = Array::from_vec(
        (0..size).map(|x| (x as f32 * 0.001).min(10.0)).collect::<Vec<_>>()
    ).reshape(&[size]);
...
fn benchmark_matrix_operations(size: usize) -> Result<()> {
    ...
    let a = Array::from_vec(
        (0..size*size).map(|x| x as f32 * 0.001).collect::<Vec<_>>()
    ).reshape(&[size, size]);
    let b = Array::from_vec(
        (0..size*size).map(|x| x as f32 * 0.002).collect::<Vec<_>>()
    ).reshape(&[size, size]);
...
fn benchmark_memory_transfer(size: usize) -> Result<()> {
    #[cfg(feature = "gpu")]
    {
        ...
        println!("Data size: {} elements ({:.2} MB)", 
                 size, 
                 (size * std::mem::size_of::<f32>()) as f64 / (1024.0 * 1024.0));
        println!("CPU → GPU transfer: {:.3} ms ({:.2} GB/s)", 
                 to_gpu_time * 1000.0,
                 (size * std::mem::size_of::<f32>()) as f64 / to_gpu_time / 1e9);
        println!("GPU → CPU transfer: {:.3} ms ({:.2} GB/s)", 
                 to_cpu_time * 1000.0,
