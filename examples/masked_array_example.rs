...
fn basic_masked_array_example() {
    ...
    println!("Filled data (masked values replaced by fill value): {:?}", filled_data);
...
fn masked_operations_example() {
    ...
    println!("Masked count: {} (zeros were automatically masked)", div_result.count_masked());
...
fn missing_data_example() {
    ...
    println!("\nData with values masked by condition (x < 3 || x > 6): {}", masked2);
    
    // Dealing with NaN and Infinity values
    let data3 = Array::from_vec(vec![1.0, f64::NAN, 3.0, f64::INFINITY, 5.0, f64::NEG_INFINITY, 7.0]);
...
fn statistical_operations_example() {
    ...
    let mask = Array::from_vec(vec![false, false, true, false, true, false, true, false, false, true]);
