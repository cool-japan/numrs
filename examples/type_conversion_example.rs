...
fn main() -> Result<()> {
    ...
    println!("\nUnsafe integer values (i32): {:?}", unsafe_values.to_vec());
    ...
    println!("\nComplex array with non-zero imaginary parts: {:?}", mixed_complex.to_vec());
    ...
    println!("\nMixed addition (int + float → f64): {:?}", addition.to_vec());
    
    let subtraction = int_array.subtract_mixed::<f64, f64>(&float_array)?;
    println!("Mixed subtraction (int - float → f64): {:?}", subtraction.to_vec());
    
    let multiplication = int_array.multiply_mixed::<f64, f64>(&float_array)?;
    println!("Mixed multiplication (int * float → f64): {:?}", multiplication.to_vec());
    
    let division = int_array.divide_mixed::<f64, f64>(&float_array)?;
    println!("Mixed division (int / float → f64): {:?}", division.to_vec());
    ...
    let bool_as_u8 = Array::from_vec(bool_array.to_vec().iter().map(|&b| if b {1u8} else {0u8}).collect::<Vec<u8>>());
